...
def build_cost_efficiency_comparison() -> dict:
    """Compare past 30-day baseline vs latest 7-day current performance."""
    data = load_data()
    daily = data["daily"]
    ...
    past_wait    = round(float(past["estimated_wait_minutes"].mean()), 1)
    current_wait = round(float(current["estimated_wait_minutes"].mean()), 1)

    past_patients    = round(float(past["appointments"].mean()), 1)
    current_patients = round(float(current["appointments"].mean()), 1)

    past_bed    = round(float(past["bed_occupancy_rate"].mean()) * 100, 1)
    current_bed = round(float(current["bed_occupancy_rate"].mean()) * 100, 1)

    past_doc_util    = round(float(past["doctor_utilization_rate"].mean()) * 100, 1)
    current_doc_util = round(float(current["doctor_utilization_rate"].mean()) * 100, 1)

    wait_change        = pct_change(past_wait, current_wait)
    patients_change    = pct_change(past_patients, current_patients)
    bed_change         = pct_change(past_bed, current_bed)
    doc_util_change    = pct_change(past_doc_util, current_doc_util)

    # Efficiency score: lower wait + lower bed occupancy = better
    ...
    return {
        ...
        "patients_per_day": {
            ...
            "change_pct": patients_change,
        },
        "bed_occupancy": {
            "past": past_bed,
            "current": current_bed,
            "change_pct": bed_change,
        },
        "doctor_utilization": {
            "past": past_doc_util,
            "current": current_doc_util,
            "change_pct": doc_util_change,
        },
